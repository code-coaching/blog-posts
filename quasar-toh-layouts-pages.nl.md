...
In dit artikel wordt de layout opgezet en gaan de verschillende pagina's toegevoegd worden. De beginsituatie is een nieuw Quasar-project. De eindsituatie is een Quasar-project waarin de layout is opgezet en waar er pagina's toegevoegd zijn.
...
In de onderstaande afbeelding zijn de verschillende pagina's te zien. De pijlen duiden aan welke knop naar welke pagina verwijst. Bijvoorbeeld, klikken op de knop met `View Details` zal ervoor zorgen dat de pagina met de details van de geselecteerde hero wordt getoond.
...
Om te weten hoe het resultaat eruitziet, kan de applicatie opgestart worden:
...
Er worden enkele `container elementen` toegevoegd om de styling van de layout en de knoppen te vergemakkelijken.
...
Een pagina wordt gekoppeld de `children`-property van de layout in het `src/router/routes.ts`-bestand. Een `child` wordt ingeladen in het `<router-view></router-view>`-element.
